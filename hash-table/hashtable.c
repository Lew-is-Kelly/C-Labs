...
struct hashtable *hashtable_new(int size)
{
  ...
  hash->table = malloc(sizeof(struct listset *) * size);
  for (int i = 0; i < size; i++)
  {
    hash->table[i] = listset_new();
  }
  hash->size = size;
  return hash;
}
...
{
  int p = hash_string(item, this->size);
  if(hashtable_lookup(this, item) == 0)
  {
    listset_add(this->table[p], item);
  }
}
...
int hashtable_lookup(struct hashtable *this, char *item)
{
  ...
  {
    if(listset_lookup(this->table[i], item) == 1)
    {
      return 1;
    }
  }
